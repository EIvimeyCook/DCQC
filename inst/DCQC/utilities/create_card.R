...
card_labels <- list(
  ...
  "1" = "Data files are accessible and in an open repository *[(click here)](https://journals.plos.org/plosone/s/recommended-repositories)*. Data must be placed in an open repository with a permanant Digital Object Identifier that is cited in the manuscript. Data must not be in the supplementary material as it does not provide a DOI",
  
  "2" = "Data are associated with a license *[(click here)](https://creativecommons.org/chooser/)*. sort. The license must be clearly visible in the repository in order to permit reuse. Legally, no license means that data cannot be legally reuused under many circumstances.",
  
  "3" = "Data files are present and complete. All files required to reproduce the results described in the manuscript must be included. Ideally, raw data alongside processed data and code should be archived.If this is not possible, a sample of raw data or a detailed description on how to obtain and prcoess the raw data is required.",
  ...
  "8" = "Code is associated with a licence *[(click here)](choosealicense.com)*. The licence must be explicitly stated and permit reuse, modification, and redistribution of the code.",
  
  "9" = "Code files are present and complete. All scripts required for data processing, analyses, and outputs must be included.",
  
  "10" = "Code is in an interoperable format. Code should be written in widely used, open languages (e.g. R, Python) and avoid proprietary code whee possible. Code must not be arhcived in a .pdf or .doc format. ",
  ...
  "14" = "Numeric results (in table or text). Reported numerical values (e.g. sample sizes, estimates, test statistics, confidence intervals, p-values) must be reproducible from the archived code and data. It should be noted if a degree of tolerance as allowed by the data editor (e.g. using Haardwicke et al (2021): reproduced– original)/original × 100)",
  
  "15" = "Figures. All figures must be reproducible from the archived code and data.It should be noted if a degree of tolerance as allowed by the data editor."
